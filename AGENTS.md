...
- Hermes Agent 在 `modules/nixos/ai-agent/hermes-agent.nix`（仅 NixOS 模块，无 HM 模块）：
  密钥走 `sops.nix` 的 `hermes-env`（OpenCode Go 订阅对应 `OPENCODE_GO_API_KEY`），模型走
  原生 `provider = "opencode-go"`；改密钥用
  `sops modules/nixos/security/secrets/hermes.yaml` 编辑后重新加密，不要动密文。
   上游是 Tier 2 无 CI 的 flake（保持独立 nixpkgs pin），升级前先查上游提交再 `nix flake lock --update-input hermes-agent`。
- Hermes 另有一个 HM 用户级入口 `modules/hm/ai-agent/hermes.nix`：从 `llm-agents`（numtide，
  独立 nixpkgs pin）的 `hermes-agent` 包安装，密钥放 `~/.hermes/.env`，升级用
  `nix flake lock --update-input llm-agents`。
- `modules/hm/ai-agent/` 与 `modules/nixos/ai-agent/` 是两套独立的东西：前者是 OpenCode 的
  HM 配置，后者是 Hermes 的 NixOS 服务，互不相干。
